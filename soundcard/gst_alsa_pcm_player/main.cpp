...
static void usage(FILE *fp, int argc, char **argv){
    fprintf (fp,
             "Usage: %s [options]\n\n"
             "Options:\n"
             "  -e | --endianness   endianness of datas[ 1234 , 4321 ] default: 1234\n"
             "  -s | --signed       signed of datas[ true , false ] default: true\n"
             "  -w | --width        width of datas default: 32\n"
             "  -d | --depth        depth of datas default: 32\n"
             "  -c | --channels     channels default: 2\n"
             "  -r | --rate         rate default 44100\n"
             "  -p | --path         filepath\n"
             "  -h | --help         print this message\n\n"
             "-- Operation must not miss path parameters. --\n"
             "-- Default value will used when parameter is missing.  --\n\n"
             "",
             basename(argv[0]));
}
...
static void *timing_thread(void *){
    ...
    while(!g_quit){
        ...
        printf("-- Playing %02d:%02d:%02d --", duration/3600, duration/60, duration%60);
        fflush(stdout);
...
int main(int argc, char **argv){
    ...
    source = gst_element_factory_make("filesrc", "_source");
    queue  = gst_element_factory_make("queue", "_queue");
    sink   = gst_element_factory_make("alsasink", "_sink");
